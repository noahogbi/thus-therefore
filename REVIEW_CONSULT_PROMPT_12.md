# Twelfth relay — final-text compliance sign-off (2026-08-21)

You are one of the two parties (Fable / Sol). Below the divider is the
assembled, near-final results post (after a third-model calibration review
and pending Noah's voice pass). This is a COMPLIANCE round, not an
editorial one. In scope: (a) whether your verbatim blocks are unedited,
(b) whether every element you ruled required is present, (c) whether the
surrounding prose misrepresents your positions, and (d) the three named
amendments below, which touch YOUR adopted text and therefore need your
explicit sign-off. Out of scope: style preferences, new requirements, and
edits to your counterpart's text. Silence on style is assent.

## The three amendments (from the calibration review)

**(12.A — Sol)** In the adopted TL;DR, replace your phrase "a validated
instrument" with "a certified instrument with documented validation
limits." Rationale: the post itself documents an audit blind spot, a
defect that passed the audit, and an unsatisfiable determinism gate;
"validated" hands a critic a rhetorical contradiction the replacement
avoids.

**(12.B — Fable)** Your 11.3(iii) required element, reworded to separate
shape mismatch from statistical resolution: "the observed point-estimate
shape matched neither registration: the aggregate departs descriptively
from flat at d6/d8 (+0.028, +0.031), though both 95% CIs include zero;
monotone growth likewise fails descriptively at d10 (−0.010) and across
the full axis (0.000)." Rationale: your original "flat fails" reads as a
significance claim adjacent to CIs containing zero.

**(12.C — Fable)** Your 10B.2 required element, reworded from "the error
structure of the registered estimates is unaffected" to the empirical
form: "the reported seed-to-seed spread includes this observed source of
numerical nondeterminism." Rationale: "unaffected" asserts exchangeability
of hardware tie-flip noise with seed variability, which was not directly
quantified.

## Response format

For each of (a)-(c): COMPLIANT, or the specific passage and the specific
misrepresentation. For your amendment(s): ACCEPT, or your counter-wording.
One round; reconciliation follows; then publication.
